...
def simulate_sip(monthly_contribution, annual_return, years,
    ...
    Returns:
        ...
        df_year                : Yearly summary DataFrame (clean 12-month buckets)
        fv_nominal             : Final nominal corpus
        fv_real                : Inflation-adjusted corpus (today's purchasing power)
    ...
    monthly_contribs = []
    dates = pd.date_range(
        start=pd.Timestamp.today().normalize(), periods=n_months, freq='ME'
    )
    ...
    # ── Yearly summary ────────────────────────────────────────────────────────
    # Group by simulation year (0-based: months 1-12 = year 1, etc.) so every
    # bucket is exactly 12 months.  Calendar-year resample("Y") creates a
    # partial final row whenever the simulation starts mid-year.
    df_monthly["sim_year"] = (df_monthly["month_index"] - 1) // 12 + 1

    df_year = (
        df_monthly
        .groupby("sim_year")
        .agg(
            annual_contribution=("monthly_contribution", "sum"),
            begin_balance      =("balance", "first"),
            end_balance        =("balance", "last"),
        )
        .reset_index()
    )

    # begin_balance of year N should be end_balance of year N-1 (i.e. before
    # the first contribution of the new year hits).  The "first" balance in
    # each group already reflects one month of growth + contribution, so we
    # shift end_balance down by one row for begin_balance instead.
    df_year["begin_balance"] = [0.0] + df_year["end_balance"].iloc[:-1].tolist()

    df_year["interest_earned"] = (
        df_year["end_balance"]
        - df_year["begin_balance"]
        - df_year["annual_contribution"]   # interest = growth minus what we put in
    )

    # Label each row with the actual calendar year it starts in
    start_year = pd.Timestamp.today().year
    df_year["year"] = start_year + df_year["sim_year"] - 1

    df_year = df_year[["year", "begin_balance", "annual_contribution", "interest_earned", "end_balance"]]
...
def find_required_monthly_sip(target_corpus, annual_return, years,
    ...
    Uses binary search — calculation logic is unchanged.
    """
